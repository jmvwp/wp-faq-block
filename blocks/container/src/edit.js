		...
		>
			<InspectorControls>
				<PanelBody title={__('Settings', 'mvwp-blocks')}>
					<PanelRow>
						<ToggleControl label={__('Show title', 'mvwp-blocks')} checked={hasTitle} onChange={onHasTitleChange} />
					</PanelRow>
					<PanelRow>
						<ToggleControl
							label={__('Show description', 'mvwp-blocks')}
					...
					<PanelRow>
						<ToggleControl
							label={__('Always open', 'mvwp-blocks')}
							help={__('Leaves items open while opening another item.', 'mvwp-blocks')}
							checked={alwaysOpen}
			...
			<div className={`${BLOCK_CLASS_NAME}__header`}>
				...
				{hasDescription && (
					<RichText
						...
						multiline="p"
						value={description}
						placeholder={__('Description', 'mvwp-blocks')}
